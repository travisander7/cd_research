...
k <- 10
p_ic_init <- 0.05
tau2 <- c(0, 0.05, 0.10, 0.20, 0.40, 0.80)
min.n <- 20
max.n <- 30
...
t_obs <- map_dbl(sim_dfs, ~ sum(.$TRT_event))
dist <- map(sim_dfs, function(df) with(df, rema::rema(TRT_event, TRT_n, CTRL_event, CTRL_n, alpha = 0.05)$dist))

# Alpha values for evaluating coverage?
